...
if ( ! class_exists( 'WCJ_Products_Add_Form_Shortcodes' ) ) :
	...
	class WCJ_Products_Add_Form_Shortcodes extends WCJ_Shortcodes {
		...
		public function wcj_product_add_new( $atts ) {
			...
			if ( isset( $_REQUEST['gallary_image_id'] ) && isset( $_REQUEST['wcj_edit_product-nonce'] ) && isset( $_REQUEST['wcj_edit_product'] ) ) {
				...
				$product      = wc_get_product( sanitize_text_field( wp_unslash( $_REQUEST['wcj_edit_product'] ) ) );

				if ( $product && $edit_wpnonce ) {
					$author_id = $product->get_post_data()->post_author;

					if ( (string) get_current_user_id() === $author_id && 'attachment' === get_post_type( sanitize_text_field( wp_unslash( $_REQUEST['gallary_image_id'] ) ) ) ) {
